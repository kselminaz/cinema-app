...
public class LanguageController {
    ...
    @PutMapping("/{id}")
    @ResponseStatus(NO_CONTENT)
    public void updateLanguage(@PathVariable Long id,@Valid @RequestBody LanguageRequest request) {
        languageService.updateLanguage(id, request);
    }

    @PatchMapping("/{id}")
    @ResponseStatus(NO_CONTENT)
    public void updateLanguageWithStatus(@PathVariable Long id, @RequestParam LanguageStatus status) {
